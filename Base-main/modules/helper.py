...
from config import USE_PROXY, GWEI_CONTROL, POVTOR_TX, PAUSA_MAX, PAUSA_MIN, MIN_ETH_ON_WALLET, USE_GAS_BASE
from modules.abi_and_contract import stable_abi, BASE_TOKENS
...
w3_base = Web3(Web3.HTTPProvider('https://rpc.ankr.com/base'))
...
def retry(func):
    def wrapper(*args, **kwargs):
        retries = 0
        while retries < POVTOR_TX:  # Задайте нужное количество попыток
            try:
                result = func(*args, **kwargs)
                return result
            except Exception as e:
                print(f"Произошла ошибка: {e}, повтор попытки {retries + 1}")
...
def cheker_gwei():
    ...
    try:
        if w3.eth.gas_price > max_gwei:
            logger.info('Газ большой, пойду спать')
            while w3.eth.gas_price > max_gwei:
                sleep(60)
...
def create_value(chain_w3, procent, from_coin, to_coin, account, popitka=0):
    ...
    if from_coin == 'ETH':
        ...
        balance_contract = chain_w3.eth.contract(address=BASE_TOKENS[from_coin], abi=stable_abi)
